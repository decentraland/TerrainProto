...
public class TerrainGeneratorWithAnalysis : MonoBehaviour
{
    ...
    // Runtime metadata
    private Dictionary<string, object> _terrainMetadata = new Dictionary<string, object>();
    ...
    public void GenerateOptimizedTerrain()
    {
        ...
        {
            previewMaterial.mainTexture = (optimizedTexture != null) ? optimizedTexture : rawTexture;
        }
    }

    void GenerateRawTerrain()
    {
    ...
    void AnalyzeTerrain()
    {
        ...
        // Step 3: Read results
        Vector4[] statsArray = new Vector4[1];
    ...
    void SaveOutputs()
    {
        ...
        if (saveCompressedData && compressedDataBuffer != null)
        {
            SaveBC4CompressedData();
        }
    ...
    void CreateTexture(ref RenderTexture texture, string name)
    {
    ...
    void SaveOptimizedTextureAsPNG()
    {
        ...
        Texture2D texture2D = new Texture2D(optimizedTexture.width, optimizedTexture.height, TextureFormat.RGBAFloat, false, true);
        ...
        Texture2D finalTexture = new Texture2D(optimizedTexture.width, optimizedTexture.height, TextureFormat.RGBAFloat, false, true);
        ...
        if (importer != null)
        {
            ...
            importer.textureCompression = UnityEditor.TextureImporterCompression.Uncompressed; // No compression
    ...
    void SaveBC4CompressedData()
    {
        uint[] compressedData = new uint[compressedDataBuffer.count * 2];
    ...
    byte[] CreateBC4DDSFile(uint[] compressedData)
    {
    ...
    void WriteCompressedDataDirect(BinaryWriter writer, uint[] compressedData)
    {
    ...
    void WriteCompressedDataYFlipped(BinaryWriter writer, uint[] compressedData, int blocksX, int blocksY)
    {
...
// Simplified editor
#if UNITY_EDITOR
[UnityEditor.CustomEditor(typeof(TerrainGeneratorWithAnalysis))]
public class TerrainGeneratorWithAnalysisEditorSimple : UnityEditor.Editor
{
    public override void OnInspectorGUI()
    {
        ...
        TerrainGeneratorWithAnalysis generator = (TerrainGeneratorWithAnalysis)target;

        if (GUILayout.Button("Generate Optimized Terrain", GUILayout.Height(30)))
        {
            generator.GenerateOptimizedTerrain();
        }
        ...
        {
            GUILayout.Label("Last Analysis Results:", UnityEditor.EditorStyles.boldLabel);
        ...
        if (generator.rawTexture != null || generator.optimizedTexture != null)
        {
            ...
            {
                UnityEditor.EditorGUI.DrawPreviewTexture(previewRect, textureToShow);
            }
        }
    }
}
